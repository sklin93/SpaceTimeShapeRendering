...
THREE.ParametricGeometries = {
	klein_complex: function (u, v) { //scale needs to be larger
		u *= 4*Math.PI;
		v *= 4*Math.PI;
		var x = Math.cos(u)*(Math.cos(0.5*u)*(Math.sqrt(2)+Math.cos(v))+Math.sin(0.5*u)*Math.sin(v)*Math.cos(v));
        var y = Math.sin(u)*(Math.cos(0.5*u)*(Math.sqrt(2)+Math.cos(v))+Math.sin(0.5*u)*Math.sin(v)*Math.cos(v));
        var z = -Math.sin(0.5*u)*(Math.sqrt(2)+Math.cos(v))+Math.cos(0.5*u)*Math.sin(v)*Math.cos(v);

        return new THREE.Vector3(x,y,z);
	},
	klein_figure8: function (u, v){
		u *= 2 * Math.PI;
		v *= 2 * Math.PI;
		var a = 3;
		var x = (a+Math.cos(0.5*u)*Math.sin(v)-Math.sin(0.5*u)*Math.sin(2*v))*Math.cos(u);
		var y = (a+Math.cos(0.5*u)*Math.sin(v)-Math.sin(0.5*u)*Math.sin(2*v))*Math.sin(u);
		var z = Math.sin(0.5*u)*Math.sin(v)+Math.cos(0.5*u)*Math.sin(2*v);

		return new THREE.Vector3(x,y,z);
	},

	klein_bottle: function ( v, u ) {
